...
Call log:
  - waiting for locator('[data-qa-id="preview-title"]').first()

    at C:\Users\Lena\OneDrive\Documents\qa-practice-pw\tests\conduit\add-comments.spec.ts:14:60
```
...
```ts
  ...
  12 |
  13 | // 
> 14 | await page.locator('[data-qa-id="preview-title"]').first().click();
     |                                                            ^ Error: locator.click: Test timeout of 30000ms exceeded.
  15 | await page.locator('//textarea').fill('Hello! Good job!');
  16 | await page.locator('button[class="btn btn-sm btn-primary"]').click();
  17 | await expect(page.getByText('Hello! Good job!').first()).toContainText('Hello! Good job!');
  18 |
  19 | });
```